---
title: Análise da Produção Científica e Acadêmica da Universidade de Brasília
author: "Antônio Henrique de Moura Rodrigues, Breno Felipe Nunes Gomes, Ícaro Nery Rezende"
date: "30/11/2019"
output:
  html_document:
    df_print: paged
...
---
```{r echo= FALSE}
# Diretórios da cada usuário
# Antônio Henrique
my_dir = "/home/ahmoura/Documents/relatorio-ds4all"
# Ícaro Rezende
# my_dir = ""
# Breno Felipe
# my_dir = ""
...
O documento a seguir apresenta o material desenvolvido pelo grupo 11 da disciplica de Tópicos avançados em Computadores - Turma D - 2018.2, do Departamento de Ciência da Computação da Univerdade de Brasília. A proposta do relatório final da  disciplina é a verificação e tratamento dos dados dos programas de pós-graduação da Universidade de Brasília, com a finalidade de se obter resultados quanto ao progresso de cada área, através da realização de análises das respectivas produções científicas e acadêmicas.

A metodologia utilizada durante a execução do relatório foi a CRISP-DM (um acrônimo para Cross Industry Standard Process for Data Mining), muito utilizada nas áreas relacionadas a ciência de dados e afins. 
...
O domínio de aplicação do projeto é o da produção científica e acadêmica brasileira, mais específicamente a produção científica ou produção acadêmica de um subgrupo de pesquisadores vinculados à Universidade de Brasília. O domínio de aplicação do projeto foi do Colégio de Humanidades, na grande área de Ciências Humanas, a área de Sociologia. Na grande área de Ciências Sociais Aplicadas, as áreas de Comunicação e Informação, e de Serviço Social.

<!--registrada na [base de currículos Lattes](lattes.cnpq.br), do CNPq.-->

### Tipo de Problema abordado

O tipo de problema abordado é o da produção de análises descritivas, quantitativas e de modelagem computacional ou estatística, que permitam caracterizar como e porque ocorre a produção científica e acadêmica de um grupo de pesquisadores. Essa caracterização visa subsidiar a tomada de decisão por membros do Sistema Nacional de Pós-Graduação.
...
* Funções das bibliotecas de ciência de dados em R propostas por Wickham e Grolemund (2016);
* Abordagem baseada em mineração de texto, para melhor analisar os dados;
* Metodologia CRISP-DM;
* O ambiente RStudio, para utilização das ferramentas citadas anteriormente.
...
### O que é a UnB?
A Universidade de Brasília, atualmente, oferece dois tipos de programa de pós-graduação, 
a pós-graduação acadêmica e a pós-graduação profissional. Quanto à rpodução acadêmica, atualmente, 
são contabilizados mais de 27 mil artigos publicados ou aceitos e mais de 23 mil orientações acadêmicas.

### Descrição das pós-graduações da UnB
Como responsabilidade do grupo, foram alocados 4 programas para serem analisados. Os programas, com 
seus subprogramas relativos são:

  * Pós-Graduação em Comunicação
    Possui uma área de concentração, Comunicação e Sociedade, que concentra as 4 linhas 
  ...
  * Pós-Graduação em Ciência da Informação
    Possui uma área de concentração, Gestão da Informação, que concetra 2 linhas de pesquisa:
      * Comunicação e Mediação da Informação
      * Organização da Informação
  * Pós-Graduação em Política Social
    Possui uma área de concentração, Estado, Políticas Sociais e Direitos, que abrange 4 linhas de pesquisa:
  ...
  * Pós-Graduação em Sociologia
    Possui uma área de concentração, Sociedade e Transformação, e 7 linhas de pesquisa:
...
Os programas de pós-graduação da Universidade de Brasília possuem, em geral uma boa avaliação 
quanto às pontuações atribuídas. Os programas considerados de excelência, que possuem nota 6 ou 7, 
possuem produções em outros idiomas, o que sugere uma conexão com centros de pesquisa internacionais.
...
#### Pós Graduação em Ciência da Informação
Na última avaliação, feita em 2017, obteve nota 5. Possui 27 docentes, 383 artigos publicados, sendo que
68 foram realizadas em 2017. 
#### Pós Graduação em Comunicação
Na última avaliação, feita em 2017, obteve nota 4. Possui 31 docentes, 309 artigos publicados, sendo que
36 foram realizadas em 2017. 
#### Pós Graduação em Política Social
Na última avaliação, feita em 2017, obteve nota 6. Possui 21 docentes, 192 artigos publicados, sendo que
25 foram realizadas em 2017. 
#### Pós Graduação em Sociologia
Na última avaliação, feita em 2017, obteve nota 7. Possui 27 docentes, 253 artigos publicados, sendo que
28 foram realizadas em 2017. 
...
```{r}
json.perfil <- "unbpos/unbpos.profile.json"
...
```{r}
json.ci.perfil <- "ciencia_informacao/ci.profile.json"
file.info(json.ci.perfil)
json.co.perfil <- "comunicacao/co.profile.json"
file.info(json.co.perfil)
json.ps.perfil <- "politica_social/ps.profile.json"
file.info(json.ps.perfil)
json.so.perfil <- "sociologia/so.profile.json"
file.info(json.so.perfil)
```

Da mesma forma, os arquivos `json.ci.perfil`, `json.co.perfil`, `json.ps.perfil` e `json.so.perfil`, apresentam, respectivamente, os dados sobre o perfil dos docentes 
vinculados programas de pós-graduação em Ciência da Informação, Comunicação, Política Social e Sociologia.

###Orientações de mestrado e doutorado realizadas pelos docentes vinculados às pós-graduações

```{r}
json.advise <- "unbpos/unbpos.advise.json"
...
```{r}
json.ci.advise <- "ciencia_informacao/ci.advise.json"
file.info(json.ci.advise)
json.co.advise <- "comunicacao/co.advise.json"
file.info(json.co.advise)
json.ps.advise <- "politica_social/ps.advise.json"
file.info(json.ps.advise)
json.so.advise <- "sociologia/so.advise.json"
file.info(json.so.advise)
```

Da mesma forma, os arquivos `json.ci.advise`, `json.co.advise`, `json.ps.advise` e `json.so.advise`, apresentam, respectivamente, os dados sobre as orientações de mestrado e doutorado feitas pelos docentes 
vinculados programas de pós-graduação em Ciência da Informação, Comunicação, Política Social e Sociologia.

###Produção bibliográfica gerada pelos docentes vinculados às pós-graduações

```{r}
json.producao.bibliografica <- "unbpos/unbpos.publication.json"
...
```{r}
json.ci.producao.bibliografica <- "ciencia_informacao/ci.publication.json"
file.info(json.ci.producao.bibliografica)
json.co.producao.bibliografica <- "comunicacao/co.publication.json"
file.info(json.co.producao.bibliografica)
json.ps.producao.bibliografica <- "politica_social/ps.publication.json"
file.info(json.ps.producao.bibliografica)
json.so.producao.bibliografica <- "sociologia/so.publication.json"
file.info(json.so.producao.bibliografica)
```

Da mesma forma, os arquivos `json.ci.producao.bibliografica`, `json.co.producao.bibliografica`, `json.ps.producao.bibliografica` e `json.so.producao.bibliografica`, apresentam, respectivamente, os dados sobre a produção bibliográfica gerada pelos docentes 
vinculados programas de pós-graduação em Ciência da Informação, Comunicação, Política Social e Sociologia.

### Agrupamento dos docentes conforme áreas de atuação

```{r}
json.researchers_by_area <- "unbpos/unbpos.researchers_by_area.json" 
...
```{r}
file.info('unbpos/unbpos.graph.json')
```

O arquivo `unbpos/unbpos.graph.json` apresenta redes de colaboração na co-autoria de artigos científicos, feitas entre os docentes vinculados a programas de pós-graduação da UnB, entre 2010 e 2017.

```{r}
file.info('ciencia_informacao/ci.graph.json')
file.info('comunicacao/co.graph.json')
file.info('politica_social/ps.graph.json')
file.info('sociologia/so.graph.json')
```
Do mesmo modo, esses arquivos apresentam redes de colaboração na co-autoria de artigos. Representando, respectivamente, os programas de pós-graduação em Ciência da Informação, Comunicação, Política Social e Sociologia.
...
```{r}
unb.prof <- fromJSON("unbpos/unbpos.profile.json")
```
O mesmo pode ser replicado para cada uma das áreas de interesse.

```{r}
ci.prof <- fromJSON("./ciencia_informacao/ci.profile.json")
co.prof <- fromJSON("./comunicacao/co.profile.json")
ps.prof <- fromJSON("./politica_social/ps.profile.json")
so.prof <- fromJSON("./sociologia/so.profile.json")
```


A quantidade de docentes sob análise é apresentada a seguir:
Quantidade de docentes na pós-graduação da UnB.
...
É possível fazer o mesmo com os dados específicos de cada programa de pós-graduação.

```{r}
# ciencia da informacao
glimpse(ci.prof[[3]], width = 30)
# comunicacao 
glimpse(co.prof[[7]], width = 30)
# politica social
glimpse(ps.prof[[8]], width = 30)
# sociologia
glimpse(so.prof[[2]], width = 30)
```
...
O que é possível gerar a partir desses dados, para o conjunto dos 1764 docentes da UnB?
...
```{r}
unb.adv <- fromJSON("unbpos/unbpos.advise.json")
names(unb.adv)
...
```
```{r}
ci.adv <- fromJSON("./ciencia_informacao/ci.advise.json")
names(unb.adv) == names(ci.adv)
co.adv <- fromJSON("./comunicacao/co.advise.json")
ps.adv <- fromJSON("./politica_social/ps.advise.json")
so.adv <- fromJSON("./sociologia/so.advise.json")
```
A mesma leitura feita com o arquivo `unb.adv` pode ser realizada com os arquivos dos programas específicos.

###Descrição dos dados de produção bibliográfica
```{r}
unb.pub <- fromJSON("unbpos/unbpos.publication.json")
names(unb.pub)
...
```
```{r}
ci.pub <- fromJSON("./ciencia_informacao/ci.publication.json")
co.pub <- fromJSON("./comunicacao/co.publication.json")
ps.pub <- fromJSON("./politica_social/ps.publication.json")
so.pub <- fromJSON("./sociologia/so.publication.json")
```
A mesma leitura feita com o arquivo `unb.pub` pode ser realizada com os arquivos dos programas específicos.

###Descrição dos dados de agregação de docentes por área
```{r}
unb.area <- fromJSON("./unbpos/unbpos.researchers_by_area.json")
...
unb.prof.json <- read_file("./unbpos/unbpos.profile.json")
unb.prof.df.capes <- read.csv("./unbpos/PesqPosCapes.csv", 
